...
class CarlaEnvironmentWrapper:
    def __init__(self, stack_frames=1, img_width=480, img_height=320, control_onput=1, rank=0, port=2000, throttle=0.35, randomization=False, preprocess="origin", render=False):
    ...
    def set_settings(self):
        ...
        if self.imagetype == "origin":
            ...
            camera = Camera('Camera', PostProcessing='Depth')
        else:
            camera = Camera('Camera', PostProcessing='SemanticSegmentation')
    ...
    def _update_state(self, action):
        ...
        if self.imagetype == "origin":
            ...
            img = image_converter.depth_to_logarithmic_grayscale(sensor_data.get('Camera', None))
        else:
            # img = image_converter.labels_to_cityscapes_palette(sensor_data.get('Camera', None))
            img = image_converter.labels_to_array(sensor_data.get('Camera', None))
